...
_isr32:
    ; Se pushean los registros de proposito general al stack.
    pushad
    ; Se le indica al PIC1 que la interrupcion que lanzo fue atendida.
    call pic_finish1

    ; Obtenemos el selector de la tarea a a la que le toca ejecutar.
    ; Si por alguna razon no tenemos tareas para ejecutar, se obtiene el 
    ; selector de la tarea idle.
    ; Por su pollo, el resultado se guarda en AX.
    call sched_next_task

    ; Carga, de TR, el selector de la tarea actual en el registro CX.
    str cx
    ; Compara el valor de los selectores.
    cmp ax, cx
    ; Si aun le toca ejecutar a la tarea actual, no la cambiamos
    je .fin

    ; Se carga el selector de la TSS de la tarea que va a pasar a ejecutarse
    ; en la posicion de memoria reservada para sched_task_selector.
    mov word [sched_task_selector], ax
    ; Se realiza un jmp far, el cual toma un valor de 48 bits. Como se definio
    ; primero el offset y justo despues el selectos, estos valores son contiguos,
    ; y como la lectura es little-edian, el jump far se termina interpretando
    ; como jmp selector:offset.
...
_isr33:
    ...
    call pic_finish1
    in al, KEYBOARD_PORT ;nos da un scan code. 7 bits para identificar tecla + bit make/break

    movzx edi, al       
    sub esp, 4
    mov [esp], edi ;;uso esta forma mecanica porque me llora el compilador
    ; 2. Leemos la tecla desde el teclado y la procesamos con la funcion tasks_input_process
    call tasks_input_process
    add esp, 4 ; en general. Alineamos a 4 bytes. 
